...
#define LW_FLAG_HAS_WAITERS			((uint32) 1 << 2)
#define LW_FLAG_RELEASE_OK			((uint32) 1 << 1)
#define LW_FLAG_LOCKED				1

#define LW_VAL_EXCLUSIVE			((uint32) 1 << 30)
#define LW_VAL_SHARED				((uint32) 1 << 6)

#define LW_LOCK_MASK				((uint32) ((1 << 25)-1) << 6)
/* Must be greater than MAX_BACKENDS - which is 2^23-1, so we're fine. */
#define LW_SHARED_MASK				((uint32) ((1 << 24)-1) << 6)
...
#define try_cmpzxadd(_ptr, _pold, _new)		        \
({									\
	int success;							\
	...
	asm volatile("cmpzxadd %[new], %[old], %[ptr]"		\
					CC_SET(z)					\
					: CC_OUT(z) (success),			\
						[ptr] "+m" (*__ptr),			\
						[old] "+a" (__old)			\
...
#define try_cmpbxadd(_ptr, _old, _new)		        \
({									\
	int success;							\
	__typeof__(*(_ptr)) __old = (_old);				\
	__typeof__(*(_ptr)) __new = (_new);				\
	volatile uint32_t *__ptr = (volatile uint32_t *)(_ptr); \
	asm volatile("cmpbxadd %[new], %[old], %[ptr]"		\
					CC_SET(b)					\
					: CC_OUT(b) (success),			\
						[ptr] "+m" (*__ptr)			\
					: [old] "r" (__old),			\
...
// 	}
	...
	if (mode == LW_EXCLUSIVE)
	{
		old_state = LW_VAL_SHARED;
		desired_state = LW_VAL_EXCLUSIVE;
	}
	else
	{
		old_state = LW_SHARED_MASK;
		desired_state = LW_VAL_SHARED;
	}

	if (try_cmpbxadd(&(&lock->state)->value, old_state, desired_state))
	{
			/* Great! Got the lock. */
		#ifdef LOCK_DEBUG
			if (mode == LW_EXCLUSIVE)
